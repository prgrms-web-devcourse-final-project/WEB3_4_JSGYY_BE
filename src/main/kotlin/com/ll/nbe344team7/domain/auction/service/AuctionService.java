    ...
    public void checkAuctionSchedule(){
        ...
        if(!auctionSchedules.isEmpty()){
            for(AuctionSchedule auctionSchedule : auctionSchedules){
                ...
                alarmService.createAlarm("'" + auction.getPost().getTitle() + "'" + WIN_BID_STRING, auction.getWinnerId(), 1);

                Member winner = this.memberRepository.findById(auction.getWinnerId()).orElse(null);
                if(winner == null){continue;}
                alarmService.createAlarm("'" + winner.getNickname() + "'" + SELLER_BID_STRING, sellerId, 1);
            }
        }
    }
}
